...
import com.fargo.basis.service.auth.AuthOrgUserManager;
import com.fargo.basis.util.list.comparator.ComparatorAuthPrivilege;
import com.fargo.basis.util.map.MapUtil;


public class AuthOrgUserCache{
	//private static AuthPrivilegeInfoCache instance;
	
	
	protected static volatile Map<String,AuthOrgUserCache> instanceMap=new Hashtable<String,AuthOrgUserCache>();
	
	public static AuthOrgUserCache getInstance(Long userId,Long library){
		synchronized(AuthOrgUserCache.class){
			String key=library+"-"+userId;
			
			if(!instanceMap.containsKey(key)){
				instanceMap.put(key, new AuthOrgUserCache(userId,library));
			}
	...
	private Long userId;
	private AuthOrgUser user;
	
	private Long library;
	public AuthOrgUserCache(Long userId1,Long library1){
		authOrgUserManager=(AuthOrgUserManager) SpringContext.getApplicationContext().getBean("authOrgUserManager");
		userId=userId1;
		library=library1;
	...
	private void init(){
		user=this.authOrgUserManager.get(userId);
