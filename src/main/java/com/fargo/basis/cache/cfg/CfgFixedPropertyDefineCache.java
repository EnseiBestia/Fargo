...
import com.fargo.basis.cache.model.FixedPropertyEnum;
...
public class CfgFixedPropertyDefineCache extends BaseCacheByLibrary<CfgFixedPropertyDefine,Long>{
	...
	public void init(){
		...
		Criteria cri = session.createCriteria(CfgFixedPropertyDefine.class);
		List<CfgFixedPropertyDefine> list=cri.list();
		if(list!=null){
	...
	private void init(CfgFixedPropertyDefine one) {
		...
		Session session = SpringContext.getHibernateSession();
		Criteria cri = session.createCriteria(CfgFixedPropertyValue.class);
		cri.add(Restrictions.eq("cfgFixedPropertyDefineId", one.getPropertyId()));
		cri.add(Restrictions.eq("library", libraryId));
		List<CfgFixedPropertyValue> list1=cri.list();
