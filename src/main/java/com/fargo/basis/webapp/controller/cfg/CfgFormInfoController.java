...
import com.fargo.basis.service.cfg.CfgFormInfoManager;
...
public class CfgFormInfoController extends BaseFormController{
    ...
    public ModelAndView list(@PathVariable String libraryPath,@PathVariable final Long parentId,HttpServletRequest request,final SearchConditionValue searchValue,@PathVariable String listFlag)throws Exception {
        ...
        try {       
        	...
        	PaginatedListHelper<CfgFormInfo> paginaredList=cfgFormInfoManager.find(currentPage,pageSize,searchValue);
    ...
    public ModelAndView onSubmit(@PathVariable String libraryPath,CfgFormInfo cfgFormInfo,@PathVariable Long parentId,  HttpServletRequest request,
        ...
        if (validator != null) { // validator is null during testing     
            ...
            try{
            	Class c=Class.forName(SpringContext.getSystemConfig().getProjectModelPackage()+cfgFormInfo.getFormCode().substring(0,1).toUpperCase()+cfgFormInfo.getFormCode().substring(1));
            	cfgFormInfo.setModelClassName(SpringContext.getSystemConfig().getProjectModelPackage()+cfgFormInfo.getFormCode().substring(0,1).toUpperCase()+cfgFormInfo.getFormCode().substring(1));
            }catch(Exception e){
                try {
                    Class c=Class.forName(SpringContext.getSystemConfig().getBasePackage()+cfgFormInfo.getFormCode().substring(0,1).toUpperCase()+cfgFormInfo.getFormCode().substring(1));
                    cfgFormInfo.setModelClassName(SpringContext.getSystemConfig().getBasePackage()+cfgFormInfo.getFormCode().substring(0,1).toUpperCase()+cfgFormInfo.getFormCode().substring(1));
                } catch (Exception e1) {
    ...
    private void deleteChildren(final Long parentId){
    	...
    	searchValue.getCustomPropertyValue().put("parentIdT", parentId);
    	List<CfgFormInfo> list=cfgFormInfoManager.find(searchValue);
    	for(int i=0;list!=null && i<list.size();i++){
    		cfgFormInfoManager.remove(list.get(0).getFormId());
    		deleteChildren(list.get(0).getFormId());
    	}
    }
