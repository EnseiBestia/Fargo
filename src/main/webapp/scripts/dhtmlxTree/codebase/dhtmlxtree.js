...
if (typeof(window.dhx4.ajax) == "undefined") {
    window.dhx4.ajax = {
        ...
        put: function(c, a, d) {
            this._call("PUT", c, a, true, d)
        },
        del: function(c, a, d) {
            this._call("DELETE", c, a, true, d)
        },
        post: function(c, a, d) {
...
dataProcessor.prototype = {
    ...
    _sendData: function(f, g) {
        ...
        if (this._tMode == "GET") {
            dhx4.ajax.get(c + ((c.indexOf("?") != -1) ? "&": "?") + this.serialize(f, g), j)
        } else {
            if (this._tMode == "POST") {
                dhx4.ajax.post(c, this.serialize(f, g), j)
            } else {
                if (this._tMode == "REST") {
                    ...
                    if (this._payload) {
                        for (var l in this._payload) {
                            e = e + (e.indexOf("?") === -1 ? "?": "&") + this.escape(l) + "=" + this.escape(this._payload[l])
                        }
                    }
                    dhx4.ajax.query({
                        url: e,
...
dhtmlXTreeObject.prototype._doContClick = function(h, a) {
    ...
    if (!this.callEvent("onRightClick", [f.id, h])) { (h.srcElement || h.target).oncontextmenu = function(l) { (l || event).cancelBubble = true;
            return false
        }
    }
    ...
    if (this._acMenu) {
        ...
        if (!_isMacOS) { (h.srcElement || h.target).oncontextmenu = function(l) { (l || event).cancelBubble = true;
                return false
            }
        }
...
dhtmlXTreeObject.prototype.preParse = function(c) {
    ...
    var a = false;
    this._p.clone().through("item", "id", c,
    function(e) {
...
function dhx_dragSomethingInTree() {
    ...
    this._preventNsDrag = function(a) { (a || window.event).cancelBubble = true;
        if ((a) && (a.preventDefault)) {
            a.preventDefault();
            return false
        }
...
dhtmlXTreeObject.prototype._editItem = function(f, a) {
    if (this._eItEd) {
        ...
        d.span.onclick = d.span.ondblclick = function(g) { (g || event).cancelBubble = true
        };
        d.span.childNodes[0].value = e;
        d.span.childNodes[0].onselectstart = function(g) { (g || event).cancelBubble = true;
            return true
        };
        d.span.childNodes[0].onmousedown = function(g) { (g || event).cancelBubble = true;
            return true
        };
        d.span.childNodes[0].focus();
        d.span.childNodes[0].focus();
        d.span.onclick = function(g) { (g || event).cancelBubble = true;
            return false
        };
        d.span.className = "";
