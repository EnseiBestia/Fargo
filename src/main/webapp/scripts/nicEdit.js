...
var nicEditorConfig = bkClass.extend({
	buttons : {
		...
		'removeformat' : {name : __('删除格式'), command : 'removeformat', noActive : true},
		'indent' : {name : __('缩进'), command : 'indent', noActive : true},
		'outdent' : {name : __('删除缩进'), command : 'outdent', noActive : true},
		'hr' : {name : __('水平线'), command : 'insertHorizontalRule', noActive : true}
	},
	iconsPath :window.document.location.pathname.substring(0,window.document.location.pathname.substr(1).indexOf('/lb')+1)+'/images/nicEditorIcons.gif',
...
var nicEditors={
	...
	allTextAreas:function(C){
		...
		for(var B=0;B<A.length;B++){nicEditors.editors.push(new nicEditor(C).panelInstance(A[B]))}
...
var nicEditor=bkClass.extend({
	...
	setPanel:function(A){this.nicPanel=new nicEditorPanel($BK(A),this.options,this);this.fireEvent("panel",this.nicPanel);return this},
	...
	selectCheck:function(C,A){
		var B=false;do{if(A.className&&A.className.indexOf("nicEdit")!=-1){return false}}while(A=A.parentNode);this.fireEvent("blur",this.selectedInstance,A);this.lastSelectedInstance=this.selectedInstance;this.selectedInstance=null;return false}});nicEditor=nicEditor.extend(bkEvent);
var nicEditorInstance=bkClass.extend({
	isSelected:false,
	construct:function(G,D,C){
		this.ne=C;
		this.elm=this.e=G;this.options=D||{};
	...
	saveContent:function(){
		if(this.copyElm||this.options.hasPanel){this.ne.fireEvent("save",this);(this.copyElm)?this.copyElm.value=this.getContent():this.e.innerHTML=this.getContent()}
	},
	getElm:function(){return this.elm},
	getContent:function(){this.content=this.getElm().innerHTML;this.ne.fireEvent("get",this);return this.content},
...
var nicEditorPanel=bkClass.extend({
	construct:function(E,B,A){
		...
		this.panelContain=new bkElement("DIV").setStyle({
			overflow:"hidden",width:"100%",border:"1px solid #cccccc",backgroundColor:"#efefef"})
			.addClass("panelContain");
		this.panelElm=new bkElement("DIV").setStyle({margin:"2px",marginTop:"0px",zoom:1,overflow:"hidden"}).addClass("panel")
...
var nicEditorPane=bkClass.extend({
	construct:function(D,C,B,A){
		...
		if(A&&!A.options.noClose){this.close=new bkElement("div").setStyle({"float":"right",height:"16px",width:"16px",cursor:"pointer"}).setStyle(this.ne.getIcon("close",nicPaneOptions)).addEvent("mousedown",A.removePane.closure(this)).appendTo(this.pane)}this.contain.noSelect().appendTo(document.body);this.position();this.init()},init:function(){},position:function(){if(this.ne.nicPanel){var B=this.ne.nicPanel.elm;var A=B.pos();var C=A[0]+parseInt(B.getStyle("width"))-(parseInt(this.pane.getStyle("width"))+8);if(C<this.pos[0]){this.contain.setStyle({left:C+"px"})}}},toggle:function(){this.isVisible=!this.isVisible;this.contain.setStyle({display:((this.isVisible)?"block":"none")})},remove:function(){if(this.contain){this.contain.remove();this.contain=null}},append:function(A){A.appendTo(this.pane)},setContent:function(A){this.pane.setContent(A)}});
...
var nicEditorSelect=bkClass.extend({
	construct:function(D,A,C,B){
		...
		this.contain=new bkElement("div").setStyle({
			width:"90px",height:"20px",cursor:"pointer",overflow:"hidden"
		})
		...
		this.control=new bkElement("div").setStyle({overflow:"hidden","float":"right",height:"18px",width:"16px"}).addClass("selectControl").setStyle(this.ne.getIcon("arrow",C))
		.appendTo(this.items);
		this.txt=new bkElement("div").setStyle({overflow:"hidden","float":"left",width:"66px",height:"14px",marginTop:"1px",fontFamily:"sans-serif",textAlign:"center",fontSize:"12px"})
		
		.addClass("selectTxt "+A).appendTo(this.items);
		if(!window.opera){
	...
	disable:function(){
		this.isDisabled=true;this.close();
	...
	update:function(A){
		var type =this.options.command;
		this.ne.nicCommand(type,A);
		this.close();
		if(type =="fontsize"){
			var Size=8;
			if(A==2)Size=10;
...
var nicLinkButton=nicEditorAdvancedButton.extend({
	addPane:function(){
		...
		this.addForm({
			...
			target:{type:"select",txt:"Open In",options:{"":"当前窗口",_blank:"新窗口"},style:{width:"100px"}}},this.ln)
		},
		submit:function(C){
...
var nicImageButton=nicEditorAdvancedButton.extend({
	addPane:function(){
		...
		this.addForm({
			...
			align:{type:"select",txt:"排列",options:{none:"默认",left:"居左",right:"居右"}}
		},this.im)
	},
	submit:function(B){
...
var nicUploadOptions = {
	buttons : {
		'upload' : {name : '上传图片', type : 'nicUploadButton2'}
	}
	
};

var nicUploadButton=nicEditorAdvancedButton.extend({
	nicURI:"/lb/impUpload/php",
	errorText:"图片上传失败",
	addPane:function(){
		if(typeof window.FormData==="undefined"){
			return this.onError("在这个浏览器不支持图片上传, 请使用Chrome,Firefox,Safari浏览器.")
		}
		this.im=this.ne.selectedInstance.selElm().parentTag("IMG");
		var A=new bkElement("div").setStyle({padding:"10px"}).appendTo(this.pane.pane);
		new bkElement("div").setStyle({fontSize:"14px",fontWeight:"bold",paddingBottom:"5px"}).setContent("选择图片").appendTo(A);
		this.fileInput=new bkElement("input").setAttributes({type:"file",name:"file",id:"file"}).appendTo(A);
	...
	setProgress:function(A){
		...
		if(A<0.98){this.progress.value=A}else{this.progress.removeAttribute("value")}
	},
...
var nicUploadButton2 = nicEditorAdvancedButton.extend({  
    serverURI :window.document.location.pathname.substring(0,window.document.location.pathname.substr(1).indexOf('/lb')+1)+'/lb/impUpload/php',
    //formElm:{html:'question'},
    ...
    addForm : function() {
        ...
        myDoc.write('<form method="post" action="'+this.serverURI+'?check='+this.myID+'" enctype="multipart/form-data">');
       ...
       // myDoc.write(this.formElm.html||'没有创建分类');
        myDoc.write('<input name="file" id="file" type="file" style="margin-top: 10px;" />');
        myDoc.write('</form>');
    ...
    startUpload : function() {
        ...
        this.myStatus.setContent('<img src="'+this.loading+'" style="float: right; margin-right: 40px;" /><strong>上传中...</strong><br />请稍等');
        this.setProgress(10);
        this.myForm.submit();
    ...
    makeRequest : function() {
        if(this.pane && this.pane.pane) {
            nicUploadButton2.lastPlugin = this;
            var s = new bkElement('script').setAttributes({ 
            	type : 'text/javascript', 
            	src : window.document.location.pathname.substring(0,window.document.location.pathname.substr(1).indexOf('/lb')+1)+
            	'/lb/impUploadForProgrees/php?check='+this.myID+'&rand='+Math.round(Math.random()*Math.pow(10,15))
...
nicUploadButton2.statusCb = function(o) {
    nicUploadButton2.lastPlugin.update(o);
};
