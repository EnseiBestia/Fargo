...
$.extend($.validator, {
	...
	messages: {
		...
		rangelength: $.validator.format("请输入一个在{0}和{1}之间的值."),
		range: $.validator.format("请输入一个在{0}和{1}之间的值."),
		max: $.validator.format("请输入一个值小于或等于{0}的值."),
	...
	prototype: {
		...
		showErrors: function( errors ,element1) {
			if ( errors ) {
				// add items to error list and map
				if(element1==null){
					element1=this.findByName(name)[0];
				}
			...
			$.ajax($.extend(true, {
				...
				success: function( response ) {
					...
					if ( valid ) {
						...
						validator.showErrors(errors,element);
					}
